...
bool LoRaManager::init()
{
  ...
  radio.startTransmit((byte[]){0x00}, 1);

  return true;
}

void LoRaManager::sendTelemetry(const SensorData &data, ProbeState state)
{
  if (transmitEnd)
  {
    transmitEnd = false;
    ...
    SerialUSB.println("Transmission complete");
  }

  if (millis() - lastTransmitted > LORA_TRANSMIT_INTERVAL && !transmitStart)

  {
    SerialUSB.println("Transmission in progress");
